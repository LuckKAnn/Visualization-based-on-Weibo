...
#打开一个excel文件
def open_xls(file):
    f = xlrd.open_workbook(file,encoding_override="utf-8")
    return f
            ...
            for j in range(len(days)):
                ...
                for row in range(num):
                        ...
                        if time ==dayTime:
                            ...
                            if sex==1:
                                tmpList['man'].append(list)
                            else:
                                tmpList['woman'].append(list)

                datavalue['data'].append(tmpList)
                # jtxt = json.dumps(datavalue)
                # print(jtxt)
                file_write_obj = open("../final/infor/infor"+str(index)+".json", 'w',encoding="utf-8")
                index+=1
                file_write_obj.write(json.dumps(datavalue,ensure_ascii=False))
